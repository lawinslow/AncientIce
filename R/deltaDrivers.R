...
# ==============================
# = Bonferonni on Paired Tests =
# ==============================
# Apply Bonferonni correction to Suwa paired comparisons (K = # tests = 5)
# Bonferonni is highly conservative
# setting # tests to 5 does imply that the tests performed for suwa are independent of those performed for tornio
suwa.bonChange <- ((iceTobit.s[,7])>=0.05/5) & ((iceTobit.s[,7])<0.05)
sum(suwa.bonChange)/2 # 1, differences in x=year no longer significant

suwa.bonChange.full <- ((iceTobit.s[,7])>=0.05/11) & ((iceTobit.s[,7])<0.05) # note that if we increase the number of tests to 11, which would suggest that we consider the suwa and tornio tests to be in the same family, then none of our conclusions about suwa pairs change
sum(suwa.bonChange.full)/2 # 2, differences in x=year and x=air.t.as no longer significant

# regardless of how stringent the correction, the changes in x=co2 are still significant


# Apply Bonferonni correction to Tornio paired comparison (K = # tests = 6)
tornio.bonChange <- ((iceTobit.t[,7])>=0.05/6) & ((iceTobit.t[,7])<0.05)
sum(tornio.bonChange)/2 # 1, the change in aod is no longer significant

# no point in applying even more conservative test, because nothing is significant for Tornio after assuming K = 6
